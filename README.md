...
- ★無限スクロール機能を自前で実装している。
  - 新しい情報のみ取得、更新し、効率的なデータ表示を行なっている。
...
- Strong Parameterやバリデーション、Cookieやcsrf_tokenを用いて、基本的なセキュリティ対策を行っている。
...
- ★無限スクロール機能を自前で実装している。
  - 新しい情報のみ取得、更新し、効率的なデータ表示を行なっている。
- JSONデータの型変換を、カスタムHooks内で一括で行なっている。
- Atomic Designを適用し、基本的なコンポーネント分割を行っている。
  - featureフォルダーを適用し、Atomic Designをより使いやすくカスタマイズしている。
...
- Cookieやcsrf_tokenを用いて、基本的なセキュリティ対策を行っている。
...
- ★GitHub ActionsによるCI/CDを実装できている(フロントエンドは自動デプロイのみ)。
...
## さらなる改善点
- カスタムHooksが一箇所のフォルダに集中しているため、Hooksを呼び出すファイルの近くに移動する
- コンポーネントの分割をさらに進める
